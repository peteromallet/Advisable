...
RSpec.describe Account do
  ...
  describe "#find_by_uid!" do
    ...
    context "when the account doesnt exist" do
      it "raises an error" do
        expect {
          Account.find_by_uid!("doesntexist")
  ...
  describe "#find_by_email!" do
    ...
    context "when the account doesnt exist" do
      it "raises an error" do
        expect {
          Account.find_by_email!("doesntexist")
        }.to raise_error(ActiveRecord::RecordNotFound)
      end
    end
  end
  ...
  describe "#find_by_airtable_id" do
    context "when passed a user airtable_id" do
      it "returns the user" do
        user = create(:user)
        expect(Account.find_by_airtable_id(user.airtable_id)).to eq(user)
      end
    end
    ...
    context "when passed a specialist airtable_id" do
      it "returns the specialist" do
        user = create(:user)
        specialist = create(:specialist)
        expect(Account.find_by_airtable_id(specialist.airtable_id)).to eq(specialist)
      end
    end

    context "when there is no match" do
      it "returns nil" do
        expect(Account.find_by_airtable_id("nope")).to be_nil
      end
    end
  end
  ...
  describe "#find_by_airtable_id!" do
    ...
    context "when the account doesnt exist" do
      it "raises an error" do
        expect {
          Account.find_by_airtable_id!("doesntexist")
        }.to raise_error(ActiveRecord::RecordNotFound)
      end
    end
  end
