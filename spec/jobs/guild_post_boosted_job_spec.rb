...
RSpec.describe GuildPostBoostedJob do
  ...
  let(:another_follower) { create(:specialist, :guild) }
  ...
  context "with boosted mailer" do
    ...
    it "does not enqueue a mail for the author of the post" do
      ...
      expect do
        enqueued_job
      end.not_to have_enqueued_mail(Guild::PostBoostMailer, :new_post).with({post: guild_post, follower_id: guild_post.specialist.id})
    end
  end

  context "with suggested_post notifications" do
    it "creates a notification for the follower's account" do
      expect do
        enqueued_job
