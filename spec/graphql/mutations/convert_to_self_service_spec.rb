...
describe Mutations::ConvertToSelfService do
  ...
  it "sets the status to Brief Pending Confirmation" do
    ...
    }.from("Project Created").to("Brief Pending Confirmation")
  end

  context "when the status is not Project Created" do
    let(:project) { create(:project, service_type:"Assisted", status: "Brief Pending Confirmation")}
  ...
  context "when the service_type is not Assisted" do
    let(:project) { create(:project, service_type: "Self-Service", status: "Project Created")}
