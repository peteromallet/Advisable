...
RSpec.describe Mutations::RequestCall do
  ...
  context "when the current user is a user" do
    ...
    it "creates a new interview, does not send message email, sends an email to specialist" do
      ...
      expect(SlackMessageJob).to have_been_enqueued.with(channel: "client_activity", text: "#{user.name_with_company} has requested a call with #{specialist.name}. (<https://advisable.com/toby/interviews/#{interview.id}|View in Toby>)").once
    end
  ...
  context "when the current user is a specialist" do
    ...
    it "creates a new interview, does not send message email, sends an email to user" do
      ...
      expect(SlackMessageJob).to have_been_enqueued.with(channel: "client_activity", text: "#{specialist.name} has requested a call with #{user.name_with_company}. (<https://advisable.com/toby/interviews/#{interview.id}|View in Toby>)").once
    end
  end
  ...
  context "when multiple accounts" do
    ...
    it "creates a new interview, does not send message emails, sends emails to every participant" do
      ...
      expect(SlackMessageJob).to have_been_enqueued.with(channel: "client_activity", text: "#{specialist.name} has requested a call with #{other.map(&:name_with_company).to_sentence}. (<https://advisable.com/toby/interviews/#{interview.id}|View in Toby>)").once
    end
  end
