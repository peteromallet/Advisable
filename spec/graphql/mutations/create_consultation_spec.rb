...
RSpec.describe Mutations::CreateConsultation do
  ...
  context "when a consultation record already exists" do
    let!(:user) { create(:user, account: create(:account, email:)) }

    before { create(:consultation, user:, status: "Request Started", specialist:) }

    it "does not create a new consultation record" do
      expect { AdvisableSchema.execute(query, context:) }.not_to change(Consultation, :count)
    end
  end

  context "when there is no existing user with the email" do
    it "creates a new user account" do
      expect { AdvisableSchema.execute(query, context:) }.to change(User, :count).by(1)
    end
  ...
  context "when a freelancers email is provided" do
    before { create(:specialist, account: create(:account, email:)) }
