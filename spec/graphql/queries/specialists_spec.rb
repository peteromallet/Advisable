...
RSpec.describe 'specialists root query', type: :system do
  ...
  context 'when an industry is provided' do
    ...
    context 'when the specialist does not have industry experience' do
    ...
    context 'when a specialist has an previous_project in that industry' do
  ...
  context 'when company_type is provided' do
    ...
    context 'when the specialist does not have experience with that company type' do
      it 'does not include the specialist' do
        response = AdvisableSchema.execute(query)
        nodes = response['data']['specialists']['nodes']
        expect(nodes).not_to include({'id' => specialist.uid})
    ...
    context 'when a specialist has an previous_project with that company type' do
      it 'includes the specialist' do
        create(
          :previous_project,
