...
namespace :production_case_study do
  task download: :environment do
    data_dir = "lib/tasks/data/case_studies"
    tables = %i[case_study_articles case_study_companies case_study_contents case_study_industries case_study_sections case_study_skills skills skill_categories skill_category_skills]
    FileUtils.mkdir_p(data_dir)
    tables.each do |table|
      `heroku pg:psql -c "\\copy (SELECT * FROM #{table}) TO #{data_dir}/#{table}.csv WITH (FORMAT CSV, HEADER TRUE, FORCE_QUOTE *)"`
    end
  end
...
def populate_embeddings
  ...
  yml.each do |data|
    ...
    CaseStudy::Embedding.create!(article_id: article[:id], data: data[:embedding])
  end
end
