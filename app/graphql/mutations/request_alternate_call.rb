...
module Mutations
  class RequestAlternateCall < Mutations::BaseMutation
    ...
    def resolve(interview:, reason: nil)
      ...
      SlackMessageJob.perform_later(channel: "client_activity", text: "#{current_user.account.name_with_company} has requested an alternate call with #{other_accounts.map(&:name_with_company).to_sentence}. (<https://app.advisable.com/toby/interviews/#{alternate.id}|View in Toby>)")
