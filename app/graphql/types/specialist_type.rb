...
module Types
  class SpecialistType < Types::BaseType
    ...
    def collaboration_types
      ::Specialist::COLLABORATION_TYPES.filter { |type| object.public_send(type) }
    end
    ...
    def agreement
      ...
      return nil if current_user.is_a?(Specialist)

      ::Agreement.latest_for(specialist: object, user: current_user)
    end
  end
end
